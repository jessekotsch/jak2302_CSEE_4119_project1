...
BR_list = sorted(list(BRs.items()), key=lambda t: t[0])
TPUT_list = sorted(list(TPUTs.items()), key=lambda t: t[0])
t_0 = BR_list[0][0]
...
data = [[util], [fair], list(zip(*smooth))]
lines = []
for i in range(0,3):
    font = { 'size' : 22}
    plt.rc('font', **font)
